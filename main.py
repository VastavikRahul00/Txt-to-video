...
async def account_login(bot: Client, m: Message):
 editable = await m.reply_text("**Hi BABY LOVE U 😘 PRESS /marty **" )
...
async def account_login(bot: Client, m: Message):
    ...
    editable = await m.reply_text("**baby batch ka  Title nhi rakhoge💔**")
    input0: Message = await bot.listen(editable.chat.id)
    raw_text0 = input0.text

    await m.reply_text("**baby quality send kro na 😗**")
...
@bot.on_message(pyrogram.filters.private & pyrogram.filters.command(["stats","status"]))
